...
## [2026-07-08] — Second Pass (Fable)

Nine-issue UX/bug pass following the mobile compatibility pass (2cc1e44). All device issues reproduced from iPhone 13 Pro screenshots, then verified against live code before fixing.

- **Annual label "/ear" → "/year"** — `render-tabs.js` allocation loop built the suffix with `viewPeriod.replace('ly','').replace('y','')`, which mangled "yearly" into "ear" (and needed a "dai"→"day" patch-up for daily). Replaced with the same lookup map whatif.js already used. All five periods verified: /day /week /fortnight /month /year.
- **Intermittent first-tap misses on the bottom bar** — iOS drops the synthetic `click` when a tap wobbles a few px (reclassified as scroll) or trips double-tap heuristics. Tabs now activate on `pointerup` with a movement/time guard (touch/pen only; `click` kept for mouse/keyboard, timestamp-deduped); nav controls get `touch-action: manipulation` and a transparent tap highlight. More sheet and hamburger unchanged.
- **Confirm popover off-screen** — `inlineConfirm()` anchored below the trigger with no vertical clamp, and the nowrap message forced the popover wider than a phone viewport (buttons pushed off-screen). Now flips above the trigger when there's no room below, clamps both axes, wraps its message (`max-width: calc(100vw - 16px)`).
- **"Add" buttons scroll to the new row (critical)** — new `revealNewRow()` in settings-events.js: after re-render, smooth-scrolls the new row into view (`block:'nearest'`, honours reduced-motion) and focuses its first input on fine-pointer devices (skipped on touch so the iOS keyboard doesn't bury the reveal). Wired to all 15 add paths, including the two where the new row is NOT the last child (tax brackets re-sort by min; categories splice before the trailing "other").
- **Bills due-date overflow** — the `<span>` grid children needed `min-width: 0`, and iOS date inputs an `appearance: none` + `max-width: 100%` (plus min-height against the empty-value collapse, left-aligned value). Also themes the previously grey iOS date control.
- **Notch white bar** — viewport meta now `viewport-fit=cover`; topbar gets `padding-top: env(safe-area-inset-top)`; `html` paints `--primary-bg-color-start` behind letterboxing/overscroll. Bottom bar additionally respects left/right insets for landscape.
- **Narrow-window regression** — pass 1 left the sidebar at 240px below 768px, squeezing content until allocation buckets overlapped. Now ≤768px starts collapsed (64px icon-only; hamburger still expands, explicit choice persisted; crossing the boundary re-applies the width default). Allocation grid: 150px track floor + `overflow: hidden` guard on buckets, single column ≤768px.
- **Number spinners (app-wide)** — hidden via `::-webkit-*-spin-button` + `appearance: textfield` (owner's call: hide, not custom steppers).
- **Thematic consistency audit (code-level, no screenshots)** — selects get a theme-coloured CSS-gradient chevron (a data: URI would violate the img-src CSP); checkboxes/radios take `accent-color: var(--accent-color)`; search inputs drop the iOS inset look; scrollbars thin + theme-tinted; a global themed `:focus-visible` ring. File inputs verified hidden; summary markers already hidden or theme-coloured.
- **Graphify** — installed natively on the host (py 3.14 + graphifyy 0.9.9 + openai pkg), graph refreshed at c53933a (322 nodes / 937 edges / 33 communities), `CLAUDE.md` generated at repo root with an architecture map from the graph.
- **Verified** — tests 122/122 (dev), production build clean, functional checks at 375/393/700/1280px: all 9 destinations reachable, add-scroll reveals rows inside the modal, confirm popover fully in-viewport, date input flush with its container, auto-collapse + tap-expand at ≤768px, allocation single-column without overlap.
