...
- **J2 — Two-tab panel.** Rebuild `#gui-settings-modal` into **Themes** (swatch grid with 4-circle previews + a "Your Themes" grid) | **Customize** (the surface above), keeping the single chokepoint. Round colour swatches.
  ...
  - ⏳ *Step 2 (next).* Replace the Colours grid with the tight base-6 + a "More Colours" expander over the full surface (text, muted, sub-heading, border, semantics…), introducing the few new overlay fields/tokens and round swatches.
